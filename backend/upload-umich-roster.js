/**
 * Upload University of Michigan Sigma Chi Roster
 *
 * This script uploads the roster for University of Michigan Sigma Chi (Theta Theta chapter).
 *
 * Requirements:
...
const csvFilePath = path.join(__dirname, 'University of Michigan Sigma Chi.csv');
...
async function uploadViaSupabase() {
  ...
  try {
    console.log('🔍 Searching for University of Michigan Sigma Chi chapter...\n');
    ...
    // Find University of Michigan Sigma Chi (Theta Theta chapter)
    const { data: chapter, error: chapterError } = await supabase
      ...
      `)
      .eq('greek_letter_name', 'Theta Theta')
      .eq('greek_organization_id', org.id)
      .single();

    if (chapterError || !chapter) {
      console.error('❌ Error finding University of Michigan chapter:', chapterError?.message || 'Not found');

      // Try alternate search by university name
      console.log('🔍 Trying alternate search by university name...\n');

      const { data: umichUniv } = await supabase
        .from('universities')
        .select('id, name')
        .ilike('name', '%Michigan%')
        .limit(5);

      if (umichUniv && umichUniv.length > 0) {
        console.log('Found universities with "Michigan" in name:');
        umichUniv.forEach(u => console.log(`  - ${u.name} (${u.id})`));
        ...
        const { data: chapters } = await supabase
          ...
          `)
          .eq('greek_organization_id', org.id)
          .in('university_id', umichUniv.map(u => u.id));

        if (chapters && chapters.length > 0) {
          console.log('\nFound Sigma Chi chapters at Michigan universities:');
    ...
    console.log('\n   (The script will find "University of Michigan Sigma Chi.csv" in the current directory)\n');
...
async function uploadViaAPI() {
  ...
  try {
    // First, get all chapters to find University of Michigan Sigma Chi
    ...
    // Find University of Michigan Sigma Chi
    const umichChapter = chaptersData.data?.find(ch =>
      (ch.chapter_name.includes('Michigan') || ch.universities?.name?.includes('Michigan')) &&
      ch.greek_organizations?.name === 'Sigma Chi'
    );

    if (!umichChapter) {
      console.error('❌ Could not find University of Michigan Sigma Chi in chapters list');
    ...
    console.log(`✅ Found chapter: ${umichChapter.chapter_name}`);
    console.log(`   Chapter ID: ${umichChapter.id}\n`);
    ...
    const uploadRes = await fetch(`${API_URL}/admin/chapters/${umichChapter.id}/paste-roster`, {
      method: 'POST',
      headers,
...
async function main() {
  ...
  console.log('University of Michigan Sigma Chi Roster Upload');
  console.log('='.repeat(60));
  console.log('\n');
