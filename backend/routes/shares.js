...
// ============================================
// POST /api/shares
// Body: { shareType, chapterId, mapConfig, purpose, expiresAt, maxViews, showFullRoster, showContactInfo, customMessage, customTitle }
router.post('/', auth, async (req, res) => {
  const client = await pool.connect();
  try {
    const {
      shareType, // 'chapter' or 'map'
      chapterId,
      mapConfig, // { filter: 'big10', state: 'PA', college: 'Penn State' }
      purpose,
      expiresAt,
      maxViews,
...
router.get('/:token', async (req, res) => {
  ...
  try {
    ...
    if (share.share_type === 'chapter') {
      ...
      if (share.show_full_roster) {
        membersQuery = `
          SELECT
            name,
            position,
            ${share.show_contact_info ? 'email, phone,' : ''}
            graduation_year,
            major,
            member_type
      ...
      const membersResult = await client.query(membersQuery, [share.chapter_id]);
...
router.post('/:token/view', async (req, res) => {
  ...
  try {
    const { token } = req.params;
    const { fingerprint } = req.body; // Browser fingerprint for unique viewer tracking
...
router.get('/my/all', auth, async (req, res) => {
  ...
  try {
    ...
    const shares = result.rows.map(share => ({
      ...share,
      shareUrl: `${process.env.FRONTEND_URL || 'http://localhost:5173'}/share/${share.share_token}`,
