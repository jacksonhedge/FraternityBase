// Generate SQL INSERT statements for Sigma Chi chapters
// Run this to create SQL file, then execute the SQL in Supabase
...
console.log(`-- SQL to insert ${chapters.length} Sigma Chi chapters`);
console.log(`-- Note: This assumes universities exist and Sigma Chi org exists`);
console.log(`-- Run this in Supabase SQL Editor\n`);

console.log(`-- Get Sigma Chi organization ID`);
console.log(`DO $$`);
console.log(`DECLARE`);
console.log(`  sigma_chi_id UUID;`);
console.log(`  univ_id UUID;`);
console.log(`BEGIN`);
console.log(`  SELECT id INTO sigma_chi_id FROM greek_organizations WHERE name = 'Sigma Chi';`);
console.log(``);

for (const chapter of chapters) {
  const univName = chapter.university_name.replace(/'/g, "''"); // Escape single quotes
  const greekName = chapter.greek_letter_name.replace(/'/g, "''");
  const chapterType = chapter.chapter_type.replace(/'/g, "''");
  const province = chapter.fraternity_province.replace(/'/g, "''");
  const city = chapter.city.replace(/'/g, "''");
  const state = chapter.state_province.replace(/'/g, "''");
  const country = chapter.country.replace(/'/g, "''");

  console.log(`  -- ${greekName} at ${univName}`);
  console.log(`  SELECT id INTO univ_id FROM universities WHERE name ILIKE '%${univName}%' LIMIT 1;`);
  console.log(`  IF univ_id IS NOT NULL THEN`);
  console.log(`    INSERT INTO chapters (greek_organization_id, university_id, chapter_name, greek_letter_name, chapter_type, fraternity_province, city, state_province, country)`);
  console.log(`    VALUES (sigma_chi_id, univ_id, '${greekName}', '${greekName}', '${chapterType}', '${province}', '${city}', '${state}', '${country}')`);
  console.log(`    ON CONFLICT (greek_organization_id, university_id) DO UPDATE SET`);
  console.log(`      chapter_name = EXCLUDED.chapter_name,`);
  console.log(`      greek_letter_name = EXCLUDED.greek_letter_name,`);
  console.log(`      chapter_type = EXCLUDED.chapter_type,`);
  console.log(`      fraternity_province = EXCLUDED.fraternity_province,`);
  console.log(`      city = EXCLUDED.city,`);
  console.log(`      state_province = EXCLUDED.state_province,`);
