...
import sponsorshipsRouter from './routes/sponsorships_enhanced';
...
// Inject Supabase client into marketplace router
...
// =============================================================================
// INTEREST SYSTEM - Companies marking interest in chapters
// =============================================================================

/**
 * POST /api/companies/interests/chapters/:chapterId
 * Mark interest in a chapter
 */
app.post('/api/companies/interests/chapters/:chapterId', async (req, res) => {
  try {
    ...
    const { data: chapter, error: chapterError } = await supabaseAdmin
      .from('chapters')
      .select('id, chapter_name, greek_organizations(name)')
      .eq('id', chapterId)
      .single();
    ...
    const { data: interest, error: interestError } = await supabaseAdmin
      .from('company_chapter_interests')
      .upsert({
        company_id: profile.company_id,
        chapter_id: chapterId,
        status: 'active',
    ...
    if (interestError) {
      console.error('Create chapter interest error:', interestError);
      return res.status(500).json({
        success: false,
    ...
    return res.status(201).json({
      success: true,
      message: `Interest in ${chapter.chapter_name} marked successfully`,
      interest
    });

  } catch (error: any) {
    console.error('Mark chapter interest error:', error);
    return res.status(500).json({
      success: false,
      error: 'Internal server error'
    });
...
/**
 * DELETE /api/companies/interests/chapters/:chapterId
 * Remove interest in a chapter
 */
app.delete('/api/companies/interests/chapters/:chapterId', async (req, res) => {
  try {
    const { chapterId } = req.params;
    ...
    if (deleteError) {
      console.error('Remove chapter interest error:', deleteError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Remove chapter interest error:', error);
    return res.status(500).json({
      success: false,
      error: 'Internal server error'
    });
...
/**
 * GET /api/companies/interests/chapters
 * Get all chapters this company has marked interest in
 */
app.get('/api/companies/interests/chapters', async (req, res) => {
  try {
    // Get the authorization token
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
    ...
    // Get all active interests with chapter details
    ...
    if (interestsError) {
      console.error('Get chapter interests error:', interestsError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Get chapter interests error:', error);
    return res.status(500).json({
      success: false,
      error: 'Internal server error'
    });
...
/**
 * GET /api/companies/my-listings
 * Get the company's brand listing/profile
 */
app.get('/api/companies/my-listings', async (req, res) => {
  try {
    // Get the authorization token
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
    ...
    // Get company profile which serves as the brand listing
    const { data: company, error: companyError } = await supabaseAdmin
      .from('companies')
      .select(`
        id,
        ...
        target_demographics,
        has_active_marketplace_subscription,
        created_at,
        updated_at
      `)
      .eq('id', profile.company_id)
    ...
    const listing = {
      ...
      deal_range: company.marketing_budget_range,
      target_audience: company.target_demographics,
      is_active: company.has_active_marketplace_subscription,
      is_featured: false, // This field doesn't exist in schema
      created_at: company.created_at,
      updated_at: company.updated_at
    };

    // Return company as a listing
    return res.status(200).json({
      success: true,
      data: [listing] // Return as array to match frontend expectations
    });
...
// =============================================================================
// ADMIN - Interest System Endpoints
// =============================================================================

/**
 * GET /api/admin/interests/fraternities
 * Get all fraternity→brand interests
 */
app.get('/api/admin/interests/fraternities', requireAdmin, async (req, res) => {
  try {
    ...
    if (error) {
      console.error('Error fetching fraternity interests:', error);
      return res.status(500).json({
        success: false,
        error: 'Failed to fetch interests'
      });
    ...
    console.error('Admin fraternity interests endpoint error:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error'
    });
...
/**
 * GET /api/admin/interests/companies
 * Get all company→chapter interests
 */
app.get('/api/admin/interests/companies', requireAdmin, async (req, res) => {
  try {
    const { status = 'active' } = req.query;
    ...
    console.error('Admin company interests endpoint error:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error'
    });
...
/**
 * GET /api/admin/interests/matches
 * Get mutual interest matches (fraternity interested in brand AND brand interested in fraternity's chapter)
 */
app.get('/api/admin/interests/matches', requireAdmin, async (req, res) => {
  try {
    // Get all active fraternity interests
    const { data: fraternityInterests, error: fratError } = await supabaseAdmin
      .from('fraternity_brand_interests')
      .select(`
        id,
        fraternity_user_id,
        ...
        status,
        notes,
        created_at as fraternity_interest_date,
        fraternity_users (
          id,
          first_name,
          last_name,
    ...
    if (fratError) {
      console.error('Error fetching fraternity interests:', fratError);
      return res.status(500).json({
        success: false,
        error: 'Failed to fetch fraternity interests'
      });
    }

    // Get all active company interests
    const { data: companyInterests, error: compError } = await supabaseAdmin
      .from('company_chapter_interests')
      .select(`
        id,
        company_id,
        ...
        status,
        notes,
        created_at as company_interest_date
      `)
      .eq('status', 'active');

    if (compError) {
      console.error('Error fetching company interests:', compError);
      return res.status(500).json({
        success: false,
        error: 'Failed to fetch company interests'
      });
    }

    // Find mutual matches
    const matches = [];
    for (const fratInterest of fraternityInterests || []) {
      const fraternityUser = fratInterest.fraternity_users;
      if (!fraternityUser?.chapter_id) continue;

      // Check if the company they're interested in is also interested in their chapter
      const matchingCompanyInterest = companyInterests?.find(
        ci => ci.company_id === fratInterest.company_id &&
              ci.chapter_id === fraternityUser.chapter_id
      );

      if (matchingCompanyInterest) {
        // It's a match! Get chapter details
        const { data: chapter } = await supabaseAdmin
          ...
          `)
          .eq('id', fraternityUser.chapter_id)
          .single();

        matches.push({
          fraternity_interest_id: fratInterest.id,
          company_interest_id: matchingCompanyInterest.id,
          fraternity_user: {
            id: fraternityUser.id,
            first_name: fraternityUser.first_name,
            last_name: fraternityUser.last_name,
            email: fraternityUser.email,
            college: fraternityUser.college,
            fraternity_or_sorority: fraternityUser.fraternity_or_sorority,
            position: fraternityUser.position
          },
          company: fratInterest.companies,
          chapter,
          fraternity_interest_date: fratInterest.fraternity_interest_date,
          company_interest_date: matchingCompanyInterest.company_interest_date,
          fraternity_notes: fratInterest.notes,
          company_notes: matchingCompanyInterest.notes,
          match_type: 'mutual'
        });
      }
    }
    ...
    console.error('Admin interest matches endpoint error:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error'
    });
